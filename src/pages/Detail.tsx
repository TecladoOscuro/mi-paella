...
export function Detail() {
  ...
  if (!recipe) {
    return (
      <div className="min-h-dvh bg-paella-bg flex items-center justify-center p-5 text-paella-text">
        <div className="text-center">
          <p className="text-paella-muted mb-3">Receta no encontrada</p>
          <button onClick={() => navigate('/')} className="text-paella-accent font-medium">
            Volver al inicio
          </button>
        </div>
      </div>
    ...
    <div className="fixed top-[calc(3rem+env(safe-area-inset-top,0px))] left-0 right-0 z-10 bg-paella-bg border-b border-paella-border">
      <div className="flex">
        <button
          ...
          className={`flex-1 py-2.5 text-sm font-medium text-center transition-colors ${activeTab === 'ingredientes' ? 'text-paella-accent border-b-2 border-paella-accent' : 'text-paella-muted'}`}
        >
          🛒 Ingredientes
        </button>
        <button
          onClick={() => onTab('preparacion')}
          className={`flex-1 py-2.5 text-sm font-medium text-center transition-colors ${activeTab === 'preparacion' ? 'text-paella-accent border-b-2 border-paella-accent' : 'text-paella-muted'}`}
        >
          🔥 Preparación
        </button>
      </div>
...
function IngredientsTab({
  ...
  const catLabel: Record<string, string> = {
    arroz: 'Arroz', proteina: 'Proteína', marisco: 'Marisco', verdura: 'Verduras',
    caldo: 'Caldo', condimentos: 'Condimentos', especial: 'Especial',
  }
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        <div className="flex bg-paella-surface rounded-lg p-0.5">
          <button onClick={() => onModeChange('lista')} className={`px-3 py-1.5 text-xs font-medium rounded-md ${mode === 'lista' ? 'bg-paella-border text-paella-text' : 'text-paella-muted'}`}>
            📋 Lista
          </button>
          <button onClick={() => onModeChange('compra')} className={`px-3 py-1.5 text-xs font-medium rounded-md ${mode === 'compra' ? 'bg-paella-green text-white' : 'text-paella-muted'}`}>
            ✅ Modo compra
          </button>
        </div>
        {mode === 'compra' && (
          <button onClick={onClear} disabled={checkedCount === 0} className={`text-xs ${checkedCount === 0 ? 'text-paella-border' : 'text-paella-muted'}`}>
            Limpiar
          </button>
        )}
      </div>
      ...
      {mode === 'compra' && (
        <div className="mb-4">
          <div className="flex justify-between text-xs text-paella-muted mb-2">
            <span>{checkedCount}/{totalIngredients}</span>
          </div>
          <div className="w-full bg-paella-surface rounded-full h-1.5">
            <div className="bg-paella-green h-1.5 rounded-full transition-all duration-300" style={{ width: `${(checkedCount / totalIngredients) * 100}%` }} />
          </div>
        </div>
      )}
      ...
      <div className="space-y-4">
        {categories.map((cat) => {
          ...
          return (
            <div key={cat}>
              <h3 className="text-xs font-semibold text-paella-muted uppercase tracking-wider mb-2">{catLabel[cat] ?? cat}</h3>
              {mode === 'lista' ? (
                <ul className="space-y-1.5">
                  {items.map((ing) => (
                    <li key={ing.id} className="text-sm flex justify-between py-1.5 border-b border-paella-border last:border-0">
                      <span>{ing.name}</span>
                ...
                <ul className="space-y-1">
                  {items.map((ing) => {
                    ...
                    return (
                      <li key={ing.id}>
                        <button onClick={() => onToggle(ing.id)} className="w-full flex items-center gap-3 py-2.5 border-b border-paella-border last:border-0 active:bg-paella-surface -mx-2 px-2 rounded-lg">
                          <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 ${checked ? 'bg-paella-green border-paella-green' : 'border-paella-border'}`}>
                            {checked && (
                          ...
                          <div className="flex-1 text-left">
                            <span className={`text-sm ${checked ? 'text-paella-muted line-through' : 'text-paella-text'}`}>{ing.name}</span>
                          </div>
...
function PreparationTab({ method, mode, onModeChange, tips }: { method: CookingMethod; mode: CookingMode; onModeChange: (m: CookingMode) => void; tips: string[] }) {
  return (
    <div>
      <div className="flex bg-paella-surface rounded-lg p-0.5 mb-5">
        <button onClick={() => onModeChange('induccion')} className={`flex-1 px-3 py-1.5 text-xs font-medium rounded-md ${mode === 'induccion' ? 'bg-paella-border text-paella-text' : 'text-paella-muted'}`}>
          ⚡ Inducción
        </button>
        <button onClick={() => onModeChange('parrilla')} className={`flex-1 px-3 py-1.5 text-xs font-medium rounded-md ${mode === 'parrilla' ? 'bg-paella-border text-paella-text' : 'text-paella-muted'}`}>
          🔥 Parrilla / BBQ
        </button>
      </div>
        ...
        {method.steps.map((step) => (
          <div key={step.order} className="flex gap-3">
            <div className="flex flex-col items-center">
              <div className="w-6 h-6 rounded-full bg-paella-accent text-white text-xs font-bold flex items-center justify-center shrink-0">
                {step.order}
              </div>
              {step.order < method.steps.length && <div className="w-0.5 flex-1 bg-paella-border my-1" />}
            </div>
            <div className="flex-1 pb-4">
              <p className="text-sm leading-relaxed">{step.description}</p>
              {(step.time || step.heatLevel) && (
                <div className="flex gap-3 mt-1.5">
                  {step.time && <span className="text-xs text-paella-muted">⏱ {step.time}</span>}
                  {step.heatLevel && <span className="text-xs text-paella-orange font-medium">🔥 {step.heatLevel}</span>}
                </div>
              )}
            </div>
          </div>
      ...
      <div className="bg-paella-surface rounded-xl p-4 mb-6">
        <h3 className="text-sm font-semibold mb-3">Fases de calor</h3>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-paella-border">
                ...
                <th className="text-left py-2 text-paella-muted font-medium">{mode === 'induccion' ? 'Inducción' : 'Parrilla'}</th>
              </tr>
            </thead>
            <tbody>
              {method.heatPhases.map((phase, i) => (
                <tr key={i} className="border-b border-paella-border last:border-0">
                  <td className="py-2 pr-2">{phase.phase}</td>
                  <td className="py-2 pr-2 text-paella-muted">{phase.time}</td>
                  <td className="py-2 font-medium">{mode === 'induccion' ? phase.induction : phase.parrilla}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {method.tips.length > 0 && (
        <div className="mb-6">
          <h3 className="text-sm font-semibold mb-3">Tips para {mode === 'induccion' ? 'Inducción' : 'Parrilla / BBQ'}</h3>
          <ul className="space-y-2">
            {method.tips.map((tip, i) => (
              <li key={i} className="flex gap-2 text-sm text-paella-muted">
                <span className="text-paella-accent shrink-0">💡</span>
      ...
      {tips.length > 0 && (
        <div className="bg-paella-card rounded-xl p-4 border border-paella-border">
          <h3 className="text-sm font-semibold mb-3">Consejos generales</h3>
          <ul className="space-y-2">
            {tips.map((tip, i) => (
              <li key={i} className="flex gap-2 text-sm text-paella-muted">
                <span className="text-paella-accent shrink-0">🥘</span>
