...
export function Home() {
  return (
    <div className="pb-(--safe-bottom)">
      <header className="sticky top-0 z-10 bg-white/90 backdrop-blur-xl border-b border-gray-100 px-5 pt-(--safe-bottom)">
        <div className="flex items-center justify-between h-12">
          <div>
            <h1 className="text-xl font-bold tracking-tight">🥘 Mi Paella</h1>
          </div>
        ...
        <div className="space-y-3">
          {recipes.map((recipe) => (
            <Link
              key={recipe.id}
              to={`/receta/${recipe.id}`}
              className="block bg-white rounded-2xl shadow-sm border border-gray-100 p-4 active:scale-[0.98] transition-transform duration-100"
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <svg
                  className="w-5 h-5 text-gray-300 mt-1 shrink-0"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  strokeWidth={2}
